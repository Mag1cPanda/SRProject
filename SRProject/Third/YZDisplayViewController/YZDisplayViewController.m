...
- (UIScrollView *)titleScrollView
{
    if (_titleScrollView == nil) {
        ...
        titleScrollView.layer.borderColor = [UIColor colorWithRed:204/255.0 green:204/255.0 blue:204/255.0 alpha:1].CGColor;
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    __weak YZDisplayViewController *weakSelf = self;
    // 点击时候需要动画
    [UIView animateWithDuration:0.25 animations:^{
        if (weakSelf.isUnderLineEqualTitleWidth) {
            self.underLine.yz_width = titleBounds.size.width;
        } else {
            self.underLine.yz_width = label.yz_width;
        }
